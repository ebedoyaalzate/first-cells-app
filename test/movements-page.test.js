...
import '../app/pages/movements-page/movements-page.js';

import sinon from 'sinon';

suite('movements-page', () => {
  let el;
  let elementShadowRoot;

  teardown(() => fixtureCleanup());

  suite('movements-page - with no configuration', () => {
    setup(async () => {
      el = await fixture(
        html`
          <movements-page></movements-page>
        `,
      );
    ...
    test('check on page leave', ()=>{
      ...
      el.onPageLeave();
      assert.equal(JSON.stringify(el.account), '{}');
    });    
  });
});
